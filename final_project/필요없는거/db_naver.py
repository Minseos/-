...
mapping = {
    "OUR948": "아워948",
    "S카페": "카페s",
    "또봉이통닭 서울도곡점": "또봉이통닭_도곡점",
    "버드나무집 서초동본점": "버드나무집_서초본점",
    "에슬로우 뱅뱅사거리점": "에슬로우커피_뱅뱅사거리점",
    "오늘은 어떤 닭": "오늘은_어떤닭",
    "통돼지두루치기김치찌개전문점": "통돼지_두루치기김치찌개",
    "영이네": "영이네추억의떡볶이"
}

# 네이버 리뷰 데이터 폴더 경로
data_folder = r"C:\fintech_service\final_project\data\리뷰추출\네이버"
...
# MySQL 연결 및 데이터 삽입
def insert_naver_reviews(folder_path):
        ...
        with connection.cursor() as cursor:
            for filename in os.listdir(folder_path):
                if filename.endswith(".csv"):
                    ...
                    # 파일명에서 가게 이름 추출
                    store_name_guess = os.path.splitext(filename)[0].replace("_네이버", "").replace("_reviews", "").strip()
                    ...
                    try:
                        ...
                        for _, row in data.iterrows():
                            sql = f"INSERT INTO `{table_name}` (platform, date, review) VALUES ('네이버', %s, %s)"
...
if __name__ == "__main__":
    insert_naver_reviews(data_folder)
